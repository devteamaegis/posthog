...
// Funnel trends-viz response shape: a flat FunnelStep[] where each entry's `data`
// holds the conversion percentage over time. The fields a tooltip / click handler
// reads are name, data, days, labels, and (for breakdown) breakdown_value.
...
// Actors returned for FunnelsActorsQuery clicks. Keyed by breakdown value (or
// '__none__') and the calendar date pulled from `funnelTrendsEntrancePeriodStart`.
