...
class UserInterviewTopic(UUIDTModel, CreatedMetaFields):
    ...
    # Latest stored test-interview transcript for this topic. There is no separate
    # IntervieweeContext / SharingConfiguration / UserInterview row for the synthetic
    # test interviewee — the test URL is derived from the topic UUID, and each completed
    # test call overwrites the fields below. `test_call_completed_at` is the
    # presence-marker; it's null until the first test call lands.
    test_transcript = models.TextField(blank=True, default="")
    test_summary = models.TextField(blank=True, default="")
    test_recording_url = models.URLField(blank=True, default="", max_length=2048)
    test_call_completed_at = models.DateTimeField(blank=True, null=True)
