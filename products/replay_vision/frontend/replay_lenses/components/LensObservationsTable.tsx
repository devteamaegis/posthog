...
export function LensObservationsTable({ lensId, tabId }: { lensId: string; tabId: string }): JSX.Element {
    ...
    const columns: LemonTableColumns<ReplayObservationApi> = [
        ...
        },
        {
            title: 'Model',
            key: 'model',
            render: (_, obs) => <span className="font-mono text-xs text-muted">{obs.lens_snapshot?.model || '—'}</span>,
        },
        {
            title: 'Created',
            key: 'created_at',
            ...
            <LemonTable
                ...
                emptyState={
                    <div className="p-6 text-center text-muted">
                        No observations yet. Observations appear here once the lens runs on a schedule, or when you
