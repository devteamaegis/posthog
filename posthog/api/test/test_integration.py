...
class TestGitHubIntegrationStateValidation:
    ...
    def test_github_finish_setup_without_state_query_uses_server_cache(
        ...
        cache.set(
            f"github_state:{self.user.id}",
            {
                "token": state_token,
                "next": f"/account-connected/github-integration?project_id={self.team.pk}&connect_from=posthog_code",
            },
            timeout=300,
        )
        ...
        response = client.post(
            f"/api/environments/{self.team.pk}/integrations/github/finish_setup/",
            {
        ...
        assert "account-connected/github-integration" in body["next"]
        assert body["integration"]["id"] == mock_integration.id
        assert cache.get(f"github_state:{self.user.id}") is None

    @patch("posthog.models.integration.GitHubIntegration.sync_repository_cache")
    def test_github_finish_setup_update_without_authorize_state_when_already_linked(
        self, mock_sync_repos, client: HttpClient
    ):
        client.force_login(self.user)
        ...
        body = response.json()
        assert body["installation_id"] == "12345"
        assert f"/project/{self.team.pk}/settings/project-integrations" in body["next"]
        mock_sync_repos.assert_called_once()

    @patch("posthog.models.integration.GitHubIntegration.sync_repository_cache")
    @patch("posthog.models.github_integration_base.GitHubIntegrationBase.verify_user_installation_access")
    def test_github_finish_setup_update_with_prepare_skips_installation_verify(
        self, mock_verify, mock_sync_repos, client: HttpClient
    ):
        client.force_login(self.user)
        Integration.objects.create(
            team=self.team,
        ...
        cache.set(
            f"github_state:{self.user.id}",
            {"token": "prepare-token", "next": next_path},
            timeout=300,
        )
        ...
        assert response.status_code == status.HTTP_200_OK
        assert response.json()["next"] == next_path
        mock_sync_repos.assert_called_once()
    ...
    def test_github_prepare_callback_stores_authorize_state(self, client: HttpClient):
        ...
        cached = cache.get(f"github_state:{self.user.id}")
